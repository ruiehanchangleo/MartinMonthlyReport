...
**Volunteer Hours (tracked translation time)**: The reports include a "Volunteer Hours in XTM" section (HTML) and sheet (Excel) showing per-volunteer **translation time** = tracked editing time in XTM (`targetStatistics.totalTime`), the same figure the UI shows as "Translation time [hh:mm:ss]". Columns: Volunteer | Language | Translation Time (h:mm:ss) | Jobs. Monthly reports also include a Year-to-Date section with a per-month breakdown and a line chart (matching the other YTD sheets). `volunteer_hours.py` pulls this from the async REST statistics report: `POST /projects/statistics?reportType=USER&startDate&endDate&linguistIds` (batched ~50 linguists; ISO-8601 dates) → `202 {processId}`, then poll `GET /projects/statistics?processId=X` (404 "Unavailable data" until ready) → `aggregatedByUser[] → projectStatistics[] → stepStatistics[] → jobStatistics[] → targetStatistics.totalTime` (**milliseconds**). It sums totalTime per user, excludes `EXCLUDED_USERS` (by username), and runs batches in parallel. `get_translation_time()` handles a single period (weekly); `get_translation_time_monthly()` fetches per month (Jan..report month) for the monthly report, and `summary_from_breakdown()` derives both the current-month and YTD summaries from that one pass. `generate_report._compute_volunteer_hours()` stores results on `self._volunteer_hours` / `_volunteer_hours_ytd` / `_volunteer_hours_ytd_breakdown`; best-effort, never blocks report generation. This is a pure REST API (no browser), so it runs unattended in the scheduled LaunchAgent jobs.

Superseded approach (login/logout time): the earlier version derived hours from PM-GUI login history via Playwright (`fetch_login_history.js`, `xtm_login_history_capture.js`, `.cache/pw-profile`). That required an interactive browser login and measured login-session time, not editing time. It's kept in the repo as history/fallback but is no longer used by the reports. `.cache/pw-profile` holds saved credentials and is gitignored — never commit it.
